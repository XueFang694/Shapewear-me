"""Connecteur Shapermint v1.1 — avec fallback HTML pour la disponibilité."""
...
class ShapermintConnector(ShopifyConnectorMixin, BaseConnector):
    ...
    def parse_product(self, url: str, data: str | dict) -> RawProduct:
        ...
        try:
            ...
            raise ConnectorParseError(f"Erreur Shapermint: {exc}", context={"url": url}) from exc
    ...
    def _parse(self, url: str, p: dict[str, Any]) -> RawProduct:
        ...
        category_raw = p.get("product_type") or next((t for t in tags if map_category_sm(t)), None)
        ...
        return RawProduct(
            ...
            extra={
                ...
                "is_best_seller": extract_best_seller_sm(tags, self._config.get("best_seller_tags")),
                "materials": materials,
