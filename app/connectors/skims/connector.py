"""Connecteur SKIMS — moteur shopify_json."""
from __future__ import annotations
from pathlib import Path
from typing import Any
from app.connectors.base import BaseConnector, Category, ConnectorMeta, RawProduct
from app.connectors.skims.mappings import (
    extract_best_seller_skims, map_category_skims,
)
...
class SkimsConnector(BaseConnector):
    ...
    def get_metadata(self) -> ConnectorMeta:
        return ConnectorMeta(name="SKIMS", slug="skims", version="1.0",
                             engine="shopify_json", base_url=self.base_url)

    def get_categories(self) -> list[Category]:
        return [
            Category(slug=s, name=s.replace("-", " ").title(),
                     url=f"{self.base_url}/collections/{s}", brand_slug="skims")
    ...
    def get_product_urls(self, category: Category) -> list[str]:
        ...
        for url in paginator.iter_pages(base):
            try:
                ...
                log.error("Erreur pagination SKIMS", url=url, error=str(exc)); break
        urls = [f"{self.base_url}/products/{h}.json" for h in handles]
        log.info("URLs SKIMS", category=category.slug, count=len(urls))
        return urls
    ...
    def parse_product(self, url: str, data: str | dict) -> RawProduct:
        ...
        try:
            ...
            raise ConnectorParseError(f"Erreur SKIMS: {exc}", context={"url": url}) from exc
